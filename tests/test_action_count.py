...
The ports are at exact parity: the FLO-602 inference actions
(list_inference_models, estimate_inference_cost) now exist in both.
PARITY_GAP = 0.
...
def test_python_port_parity_gap_is_documented() -> None:
    ...
    The gap is 0 (exact parity). If a new action lands in TypeScript ahead
    of Python (or vice versa), update the constants at the top of this file
    AND floe-labs-docs in the same PR.
    """
    total = _count_actions(FloeActionProvider) + _count_actions(X402ActionProvider)
